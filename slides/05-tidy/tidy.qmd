...
* Interface with databases, e.g., MySQL, DuckDB, Google BigQuery, and Apache Spark.
...
- [readr](https://github.com/tidyverse/readr) package implements functions that ingest flat text files into tibbles.
...
- An example file [heights.csv](https://raw.githubusercontent.com/ucla-biostat-203b/2026winter/master/slides/05-tidy/heights.csv):
```{bash}
head heights.csv
```
...
- Read from a local file [heights.csv](https://raw.githubusercontent.com/ucla-biostat-203b/2026winter/master/slides/05-tidy/heights.csv):
```{r}
heights <- read_csv("heights.csv") |> 
  print(width = Inf)
```

- `read_csv` can read gz file directly (HW2).

- Read from a url:
```{r}
heights <- read_csv("https://raw.githubusercontent.com/ucla-biostat-203b/2026winter/master/slides/05-tidy/heights.csv") |>
  print(width = Inf)
```
...
url = "https://raw.githubusercontent.com/ucla-biostat-203b/2026winter/master/slides/05-tidy/heights.csv"
...
http_response = HTTP.get("https://raw.githubusercontent.com/ucla-biostat-203b/2026winter/master/slides/05-tidy/heights.csv");
...
[Posit readr cheat sheet](https://rstudio.github.io/cheatsheets/html/data-import.html) is extremely helpful. 
...
- `pivot_longer` supersedes the old `gather` function.
...
- We can `pivot_longer` table4b too and then join them
...
- `pivot_wider` supersedes the old `spread` function.
...
```{r}
table3 |> 
  separate(rate, sep = "/", into = c("cases", "population"), convert = TRUE)
```
...
# After separation
table3 |> 
  separate_longer_delim(rate, delim = "/")
```
...
### Missing values and/or duplicate rows
...
- To fill in NA's using the previous (LOCF) or next value.
...
```{r}
df |> fill(c(year, cases), .direction = "down")
```
...
- To replace NA's by a specific value.
...
```{r}
df |> replace_na(list(year = 1999, cases = 500))
```
...
[Posit tidyr cheat sheet](https://rstudio.github.io/cheatsheets/html/tidyr.html) is extremely helpful. 

## Some practical decisions when ingesting data
...
- Pivot wider or longer, if needed by downstream analysis.
