...
In this tutorial, we show how to push the [rocker/rstudio](https://hub.docker.com/r/rocker/rstudio/tags) Docker image to GCR (Google Cloud Registry) and run the Docker container on GCP (Google Cloud Platform).
...
- Download the Docker image `rocker/rstudio`:
```{bash}
#| eval: false
docker pull --platform linux/amd64 rocker/rstudio
```
Why do we pull the `linux/amd64` image?

- List existing Docker images and tag for gcr:
```{bash}
#| eval: false
docker image list
docker tag rocker/rstudio gcr.io/biostat-203b-2025-winter/rstudio
```

- Push the image to gcr:
```{bash}
#| eval: false
docker push gcr.io/biostat-203b-2025-winter/rstudio
```
...
- Paste the GCR image name `gcr.io/biostat-203b-2025-winter/rstudio` to the `Container image` field of the GCE instance.
